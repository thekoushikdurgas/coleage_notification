...
from app.database import db, Organization, Notification, CrawlerLog, ScheduledTask, ScheduledTaskRun, SystemSetting
from app.scraper import ScraperEngine
from app.ai_engine import AIEngine

# Queues
scrape_queue = queue.Queue()
scraped_queue = queue.Queue()
...
def save_and_broadcast_crawler_log(org_id, status, detected_changes=False, error_message=None, task_id=None, run_id=None):
    """
...
def check_run_completion(run_id):
    ...
    try:
        ...
        if processed_count >= run.targets_count:
            ...
            # Aggregate stats
            successes = CrawlerLog.query.filter_by(run_id=run_id, status='success').count()
            failures = CrawlerLog.query.filter_by(run_id=run_id, status='failed').count()
    ...
    def run(self):
        ...
        while self.running:
            ...
            with self.app.app_context():
                org = Organization.query.get(org_id)
                if not org:
                    log_to_workers(f"Organization ID {org_id} not found in database.", "error")
            ...
            log_to_workers(f"Starting simulated scrape for college: {org_name}", "system")
            ...
            try:
                ...
                if scraped_data:
                    log_to_workers(f"Successfully scraped '{scraped_data['title'][:40]}...'. Passing to Loader Worker.", "success")
                    scraped_queue.put((org_id, task_id, run_id, scraped_data))
                else:
                    log_to_workers(f"Crawl completed for '{org_name}'. No new notices found.", "info")
                    with self.app.app_context():
                        save_and_broadcast_crawler_log(org_id=org_id, status='success', detected_changes=False, task_id=task_id, run_id=run_id)
                        if run_id:
                ...
                traceback.print_exc()
                with self.app.app_context():
                    save_and_broadcast_crawler_log(org_id=org_id, status='failed', error_message=str(e), task_id=task_id, run_id=run_id)
                    if run_id:
                        check_run_completion(run_id)
            ...
            with self.app.app_context():
                try:
                    ...
                    for item in items:
                        ...
                        org = Organization.query.get(org_id)
                        if not org:
                            log_to_workers(f"Loader error: Org ID {org_id} not found.", "error")
                            continue
                        ...
                        log_to_workers(f"Processing and loading announcement to DB: '{scraped_data['title'][:50]}...'", "system")
                        ...
                        if existing:
                            log_to_workers(f"Notice already exists in database. Skipping duplicate save.", "info")
                            log = CrawlerLog(
                                organization_id=org_id,
                                status='success',
                            ...
                            new_notif = Notification(
                                ...
                                status='Duplicate' if is_dup else ('Published' if auto_approve else 'Pending'),
                                task_id=task_id,
                                run_id=run_id
                            )
                    ...
                    for notif_info in notifications_to_broadcast:
                        notif_payload = {
                            ...
                            "created_at": notif_info["created_at"].strftime("%I:%M:%S %p")
                        }
                        broadcast_sse("notification", notif_payload)
                        
                except Exception as batch_error:
                    db.session.rollback()
                    log_to_workers(f"Batch processing error: {str(batch_error)}. Falling back to individual processing.", "error")
                    traceback.print_exc()
                    ...
                    for item in items:
                        ...
                        try:
                            ...
                            if existing:
                                log = CrawlerLog(organization_id=org_id, status='success', detected_changes=False, task_id=task_id, run_id=run_id)
                                db.session.add(log)
                                db.session.commit()
                                save_and_broadcast_crawler_log(org_id=org_id, status='success', detected_changes=False, task_id=task_id, run_id=run_id)
                            else:
                                ...
                                if not is_dup:
                                    ...
                                    if new_notif.status == 'Pending':
                                        broadcast_sse("notification", {
                                            ...
                                            "category": new_notif.category,
                                            "organization_name": org_name,
                                            "created_at": new_notif.created_at.strftime("%I:%M:%S %p")
                                        })
                                
                                db.session.commit()
                                save_and_broadcast_crawler_log(org_id=org_id, status='success', detected_changes=True, task_id=task_id, run_id=run_id)
                                
                            if run_id:
                                check_run_completion(run_id)
                        except Exception as ind_error:
                            db.session.rollback()
                            log_to_workers(f"Individual fallback error for '{org_name}': {str(ind_error)}", "error")
                            save_and_broadcast_crawler_log(org_id=org_id, status='failed', error_message=str(ind_error), task_id=task_id, run_id=run_id)
                            if run_id:
                                check_run_completion(run_id)
            ...
            try:
                with self.app.app_context():
                    ...
                    for task in due_tasks:
                        ...
                        if orgs:
                            log_to_workers(f"Scheduler: Task '{task.name}' is due. Created run #{run_id}. Queuing {len(orgs)} organizations in background.", "system")
                            ...
                            task.next_run_at = calculate_next_run(task.frequency, now)
                            db.session.commit()
                            log_to_workers(f"Scheduler: Task '{task.name}' is due. Created run #{run_id} (0 targets matched). Rescheduled.", "info")
            except Exception as e:
