...
from app.database import db, Organization, Notification, GeneratedContent, Subscription, CrawlerLog, DeliveryLog, ScheduledTask, ScheduledTaskRun, SystemSetting
...
def dispatch_alerts(notification_id):
    ...
    for sub in all_subs:
        match_org = sub.organization_id is None or sub.organization_id == notif.organization_id
    ...
    for sub in matching_subs:
        ...
        for channel in channels:
            ...
            recipient = sub.email if channel == 'email' or channel == 'push' else (sub.phone or "Simulated Phone")
            log = DeliveryLog(
    ...
    try:
        ...
        for sub in matching_subs:
            ...
            for channel in channels:
                recipient = sub.email if channel == 'email' or channel == 'push' else (sub.phone or "Simulated Phone")
                msg_text = content.meta_description if content else "Announcement posted!"
                if content:
                    if channel == 'whatsapp':
        ...
        broadcast_sse("published_notification", {
            "id": notif.id,
            "title": notif.title,
            ...
            "body_preview": (notif.body[:150] + "...") if len(notif.body) > 150 else notif.body
        })
...
def index():
    ...
    query = Notification.query.join(Organization).options(
    ...
    states = db.session.query(Organization.state).filter(Organization.state != None).distinct().order_by(Organization.state).all()
    ...
    ).join(Notification).filter(Notification.status == 'Published').group_by(Organization).order_by(db.desc('notif_count')).limit(5).all()
...
def notification_detail(seo_url):
    ...
    related = Notification.query.join(Organization).filter(
        Notification.id != notif.id,
        Notification.status == 'Published',
        Notification.is_duplicate == False,
        (Notification.category == notif.category) | (Organization.state == org.state)
    ).limit(3).all()
...
def subscribe():
    ...
    if request.method == 'POST':
        ...
        flash("Successfully subscribed! You will receive alerts when matching announcements are published.", "success")
    ...
    colleges = Organization.query.filter_by(category='college').order_by(Organization.name).limit(100).all()
...
def student_inbox():
    ...
    delivery_logs = DeliveryLog.query.options(
        joinedload(DeliveryLog.notification).joinedload(Notification.organization),
        joinedload(DeliveryLog.notification).joinedload(Notification.generated_content)
    ).order_by(DeliveryLog.sent_at.desc()).limit(100).all()
...
def check_role():
    ...
    if request.path.startswith('/admin'):
        ...
        if role == 'student':
            flash("Access denied: Students are not authorized to access administrative panels.", "danger")
            return redirect(url_for('main.index'))
            
        if role == 'moderator':
            # Moderators cannot run crawls, create/edit/delete/toggle tasks or colleges
            restricted_keywords = ['/crawl', '/bulk-crawl', '/create', '/edit', '/delete', '/toggle', '/run/']
            if any(kw in request.path for kw in restricted_keywords):
                flash("Access denied: Moderators cannot perform configuration or manual trigger actions.", "warning")
...
def admin_stream_events():
    ...
    """
    from app.workers import sse_listeners, listeners_lock, scraper_status, loader_status, scrape_queue, scraped_queue
...
def admin_dashboard():
    ...
    counts = db.session.query(Organization.category, db.func.count(Organization.id)).group_by(Organization.category).all()
    ...
    from sqlalchemy.orm import joinedload
    moderation_queue = Notification.query.options(
        joinedload(Notification.organization),
        joinedload(Notification.generated_content)
    ).filter_by(status='Pending').order_by(Notification.created_at.desc()).all()
    
    # Recent Crawl Logs
    recent_crawls = CrawlerLog.query.options(
        joinedload(CrawlerLog.organization)
    ).order_by(CrawlerLog.crawled_at.desc()).limit(15).all()
    ...
    organizations = orgs_query.order_by(Organization.category, Organization.name).limit(50).all()
...
def manual_crawl(org_id):
    ...
    log_to_workers(f"Manual crawl request received for '{org.name}' and queued.", "info")
    broadcast_status()
...
def bulk_crawl():
    ...
    if not all_orgs:
        return jsonify({'success': False, 'message': 'No registered organizations found.'})
    ...
    log_to_workers(f"Bulk crawl request received. Queued 5 organizations in background.", "info")
    broadcast_status()
...
def approve_notification(notif_id):
    ...
    flash(f"Notification '{notif.title}' approved and published. {alerts_sent} alerts dispatched to subscribers.", "success")
    return redirect(url_for('main.admin_dashboard'))
...
def approve_all_notifications():
    ...
    flash(f"Successfully approved all {total_approved} pending notifications. {total_alerts_sent} alerts dispatched to subscribers.", "success")
    return redirect(url_for('main.admin_dashboard'))
...
def ajax_approve_notification(notif_id):
    ...
    if notif.status != 'Pending':
        return jsonify({'success': False, 'message': 'Notification is not in Pending state.'})
...
def api_pending_notifications():
    ...
    for n in pagination.items:
        items.append({
            'id': n.id,
            'title': n.title,
            'body_preview': (n.body[:200] + '...') if n.body and len(n.body) > 200 else (n.body or ''),
            'category': n.category,
            'organization_name': n.organization.name if n.organization else 'Unknown',
            'organization_state': n.organization.state if n.organization else '',
            'organization_category': n.organization.category if n.organization else '',
...
def ajax_approve_all_notifications():
    ...
    pending_notifs = Notification.query.filter_by(status='Pending').all()
    if not pending_notifs:
        return jsonify({'success': False, 'message': 'No pending notifications to approve.'})
...
def admin_colleges():
    ...
    states = db.session.query(Organization.state).filter(
    ...
    if category_f in ['college', 'standalone']:
        types = db.session.query(Organization.college_type).filter(
            Organization.category == category_f,
            Organization.college_type != None
        ).distinct().order_by(Organization.college_type).all()
...
def admin_college_create():
    ...
    if request.method == 'POST':
        ...
        college_type = request.form.get('college_type', '').strip() if category in ['college', 'standalone'] else None
        management = request.form.get('management', '').strip() if category in ['college', 'standalone'] else None
        university_name = request.form.get('university_name', '').strip() if category == 'college' else None
        ...
        if aishe_code:
            ...
            if existing:
                flash(f"AISHE Code '{aishe_code}' already exists for: {existing.name}", "error")
                return redirect(url_for('main.admin_college_create'))
        else:
...
def admin_college_edit(org_id):
    ...
    if college.category not in ['college', 'standalone', 'university']:
        flash("Target record is not a college, standalone, or university organization!", "error")
    ...
    if request.method == 'POST':
        ...
        db.session.commit()
        flash(f"{category.capitalize()} '{college.name}' details updated successfully.", "success")
        return redirect(url_for('main.admin_colleges', category=category))
...
def bulk_scrape_colleges():
    ...
    log_to_workers(f"Bulk scrape request for {valid_count} organizations queued in background.", "info")
    broadcast_status()
...
def admin_task_run(task_id):
    ...
    if task.target_type == 'all':
        ...
        orgs = Organization.query.filter_by(is_tracked=True, state=task.target_query).all()
    elif task.target_type == 'selected':
        try:
            ids = [int(i) for i in task.target_query.split(',') if i.strip()]
            orgs = Organization.query.filter(Organization.id.in_(ids), Organization.is_tracked == True).all()
        except Exception:
            orgs = []
    else:
        orgs = []
    ...
    log_to_workers(f"Scheduled task '{task.name}' triggered manually. Created run #{run_id}. Queuing {len(orgs)} organizations in background.", "system")
    broadcast_status()
    
    flash(f"Scheduled task '{task.name}' triggered successfully. Created run #{run_id} and queued {len(orgs)} crawl jobs in background.", "success")
    return redirect(url_for('main.admin_tasks'))
...
def admin_task_details(task_id):
    ...
    runs = ScheduledTaskRun.query.filter_by(task_id=task.id).order_by(ScheduledTaskRun.started_at.desc()).all()
    
    selected_run_id = request.args.get('run_id', type=int)
    selected_run = None
    
    if selected_run_id:
        selected_run = ScheduledTaskRun.query.filter_by(id=selected_run_id, task_id=task.id).first_or_404()
        
        # Calculate metrics for this specific run
        total_crawls = CrawlerLog.query.filter_by(run_id=selected_run.id).count()
        success_crawls = CrawlerLog.query.filter_by(run_id=selected_run.id, status='success').count()
        failed_crawls = CrawlerLog.query.filter_by(run_id=selected_run.id, status='failed').count()
        success_rate = round((success_crawls / total_crawls * 100), 1) if total_crawls > 0 else 0.0
        
        changes_detected = CrawlerLog.query.filter_by(run_id=selected_run.id, detected_changes=True).count()
        
        notifications = Notification.query.filter_by(run_id=selected_run.id).order_by(Notification.created_at.desc()).all()
        total_notifications = len(notifications)
        
        alerts_sent = DeliveryLog.query.join(Notification).filter(Notification.run_id == selected_run.id).count()
        
        recent_logs = CrawlerLog.query.filter_by(run_id=selected_run.id).order_by(CrawlerLog.crawled_at.desc()).limit(100).all()
    else:
        # Calculate overall metrics across all time for this task
        total_crawls = CrawlerLog.query.filter_by(task_id=task.id).count()
        success_crawls = CrawlerLog.query.filter_by(task_id=task.id, status='success').count()
        failed_crawls = CrawlerLog.query.filter_by(task_id=task.id, status='failed').count()
        success_rate = round((success_crawls / total_crawls * 100), 1) if total_crawls > 0 else 0.0
        
        changes_detected = CrawlerLog.query.filter_by(task_id=task.id, detected_changes=True).count()
        
        notifications = Notification.query.filter_by(task_id=task.id).order_by(Notification.created_at.desc()).all()
        total_notifications = len(notifications)
        
        alerts_sent = DeliveryLog.query.join(Notification).filter(Notification.task_id == task.id).count()
        
        # Fetch recent crawl logs (cap at 50 for overall view to prevent performance lag)
        recent_logs = CrawlerLog.query.filter_by(task_id=task.id).order_by(CrawlerLog.crawled_at.desc()).limit(50).all()
    ...
    if task.target_type == 'all':
        ...
        target_orgs = Organization.query.filter_by(is_tracked=True, state=task.target_query).limit(20).all()
        target_count = Organization.query.filter_by(is_tracked=True, state=task.target_query).count()
    elif task.target_type == 'selected':
        try:
            ids = [int(i) for i in task.target_query.split(',') if i.strip()]
