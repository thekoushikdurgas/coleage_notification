...
class Organization(db.Model):
    ...
    category = db.Column(db.String(50), nullable=False, index=True)  # 'college', 'board', 'regulatory_body'
    ...
    college_type = db.Column(db.String(100), nullable=True)  # 'Affiliated College', etc.
    management = db.Column(db.String(100), nullable=True)  # 'Government', 'Private Un-Aided', etc.
    university_name = db.Column(db.String(255), nullable=True)
    is_tracked = db.Column(db.Boolean, default=True)
    
    # Relationships
    notifications = db.relationship('Notification', backref='organization', lazy=True, cascade="all, delete-orphan")
    crawler_logs = db.relationship('CrawlerLog', backref='organization', lazy=True, cascade="all, delete-orphan")
...
class Notification(db.Model):
    ...
    category = db.Column(db.String(50), nullable=False, index=True)  # 'admission', 'exam', 'result', 'policy', 'scholarship', 'other'

    status = db.Column(db.String(50), default='Pending', index=True)  # 'Pending', 'Published', 'Duplicate'
    ...
    duplicate_of_id = db.Column(db.Integer, db.ForeignKey('notifications.id'), nullable=True)
    task_id = db.Column(db.Integer, db.ForeignKey('scheduled_tasks.id'), nullable=True, index=True)
    run_id = db.Column(db.Integer, db.ForeignKey('scheduled_task_runs.id', ondelete='SET NULL'), nullable=True, index=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow, index=True)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    
    # Relationships
    generated_content = db.relationship('GeneratedContent', backref='notification', uselist=False, lazy=True, cascade="all, delete-orphan")
    delivery_logs = db.relationship('DeliveryLog', backref='notification', lazy=True, cascade="all, delete-orphan")
...
class GeneratedContent(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    notification_id = db.Column(db.Integer, db.ForeignKey('notifications.id'), nullable=False, unique=True, index=True)
...
class DeliveryLog(db.Model):
    ...
    channel = db.Column(db.String(50), nullable=False)  # 'email', 'whatsapp', 'telegram', 'push', 'sms'
...
class ScheduledTask(db.Model):
    ...
    runs = db.relationship('ScheduledTaskRun', backref='scheduled_task', lazy=True, cascade="all, delete-orphan")
...
class ScheduledTaskRun(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    task_id = db.Column(db.Integer, db.ForeignKey('scheduled_tasks.id', ondelete='CASCADE'), nullable=False, index=True)
